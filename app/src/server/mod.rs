...
/// Adds an example to the .ics calendar endpoint response in the OpenAPI spec.
/// This function uses string manipulation and assumes a specific structure
/// for the relevant part of the JSON generated by poem_openapi.
fn add_ics_example(json: &str) -> String {
    // Define the exact block to find, including whitespace from the curl output.
    // Note the use of raw string literal r#"..."# to avoid escaping backslashes within the string.
    let pattern_to_replace = r#""text/calendar": {
                "schema": {
                  "type": "string"
                }
              }"#;
    ...
    let ics_example = include_str!("./domains.ics")
        .replace("\\n", "\\\\n")
        .replace("\n", "\\n")
        .replace("\r", "");
...
pub async fn start_http(state: AppState) {
    ...
    let mut spec_json = reorder_openapi_tags(&spec_json, tag_order);

    // Add the .ics example
    spec_json = add_ics_example(&spec_json);
